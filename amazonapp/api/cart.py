...
def add_to_cart(name):
    ...
    cart = frappe.get_all(
        "Cart", filters={"user": user, "is_completed": False}, fields=["name"]
    )
    if cart:
    ...
    existing_item = frappe.get_all(
        "Cart Items",
        filters={"parent": cart_name, "item": name},
    ...
    cart_items = frappe.get_all(
        "Cart Items", filters={"parent": cart_name}, fields=["quantity", "price"]
    )
    ...
    frappe.db.set_value(
        "Cart", cart_name, {"total_items": total_items, "total_amount": total_amount}
    )
    frappe.db.commit()
...
def update_quantity(item_name, quantity):
    ...
    cart = frappe.get_doc("Cart", {"user": frappe.session.user, "is_completed": False})

    # Find the item in the cart
    item = next((item for item in cart.items if item.item == item_name), None)
    ...
    if item:
        ...
        new_total = sum(item.price * int(item.quantity) for item in cart.items)
...
def remove_item_from_cart(item_name):
    try:
        ...
        return {
            ...
            "new_total_items": cart.total_items,
        }
    except Exception as e:
